...
{\bf We refer to this setup as the Level4 resolution. 
Six of these haloes were also simulated at a higher Level3 resolution
that represent an increase by a factor of 8 in mass and 2 in spatial
resolution.  
We use the higher resolution haloes to define a minimal radius above
which our results can be considered as converged.} 
...
\caption{Axial ratios for the six halos simulated at two different
  resolutions with two different physics (DMO and MHD). 
  In DMO simulations there is a good agreement between the two
  ...
  $\approx 10$ kpc. 
  In this paper we only report results at scales larger $\approx 16$ kpc
  ($R_{200}/16$).}
...
Au-30 & 0.62 & 0.44 & 0.77 & 0.54 & 0.87 & 0.58 & 0.85 & 0.58 & 0.75 & 0.53\\
  \hline\hline
  \end{tabular}
  \caption{Axial ratios of Auriga dark matter halos at $z=0$ at
    different radii in DMO simulations. The subscripts $16$, $8$, $4$, $2$, $1$,
    indicate that the shape was measured at radii of $R_{200}/16$,
    $R_{200}/8$, $R_{200}/4$, $R_{200}/2$, $R_{200}$, respectively.   \label{table:DMO}}
\end{table*}
...
Au-30 & 0.89 & 0.73 & 0.78 & 0.64 & 0.69 & 0.53 & 0.80 & 0.58 & 0.91 & 0.63\\
  \hline\hline
  \end{tabular}
  \caption{Same as Table \ref{table:DMO} for MHD simulations.   \label{table:MHD}}
\end{table*}
...
\caption{Axial ratios for all simulated halos.
  ...
  These three points are further clarified in Figure 
  The first and second points are claryfied in Figure
  \ref{fig:triaxial_cumulative} through cumulative triaxiality
  distributions}
...
\caption{Triaxiality cumulative distribution at five different radii.
  Right/middle panel corresponds to the cumulative distribution of
  triaxiality in DMO/MHD simulations, respectively. 
  ...
  The right panel, shows the cumulative distribution for the changes
  ...
  more triaxial (but only at the smaller radii).} 
...
\begin{figure*}
\begin{center}
\includegraphics[width=0.9\textwidth]{angles_alignment_DM.pdf}
...
\caption{\textbf{RP: I think the upper row that compares the shape of the halo from the DMO sim with the orientation of the disk in the MHD sim is hard to interpret. Maybe instead show in the first row the angle against a fixed axis (e.g. total angular momentum of the halo?)} Angles between the principal axis of the dark matter halo
  ...
  describing the shape are constant as a function of radius except two
  cases. 
  ...
  Furthermore, the dark matter shells twists significantly in $1/5$ of
...
\caption{Twisting in the halo-disc alignment (measured as the standard deviation of the alignment angles)
  ...
  The label with the $\rho$ value corresponds to the Spearmans rank
  correlation coefficient (mean value and uncertainty estimated via
  jackknife resampling) while the line shows the result of the
...
\caption{Twisting in the halo-disc alingment as a function of the same
  ...
  a strong correlation, albeit weaker than what they had with the triaxiality.}
...
\caption{Twisting in the halo-disc alingment as a function of the
  ...
  dark matter halo (time at which it has half of its present mass).
  The label with the $\rho$ value corresponds to the Spearmans rank
  correlation coefficient (mean value and uncertainty estimated via
  jackknife resampling).
  The line is the best minimum squares fit to a line.}
...
between $1$ kpc and $R_{200}$ where  $R_{200}$ corresponds to the radius
computed enclosing a sphere with 200 times the critical density of the
Universe. 
{\bf However, this scale is similar to the gravitational smoothing length
for stellar particles.
For this reason we use the Level3 simulations performed at higher
...
available both at Level3 and Level4 resolutions.
The dotted lines compare the DMO simulations showing that down to
scales of $1$ kpc the two resolutions yield comparable results
\citep{VeraCiro11}. 
In MHD simulations the situation is different, as expected. 
The shape ratios $b/a$ and $c/a$ start to differentiate
from each other at radii smaller than $15$ kpc, tith the exception of
Au-27 that shows a good agreement in the shape measuremnts at all radii, 
Above $15$ kpc, the differences between the triaxiality values at
Level4 and Level3 in MHD simulations are small and never exceed $0.1$. 
For this reason, in order to make the DM halo shape results comparable
in DMO and MHD simulations we only report measurements above $15$
kpc.}
...
halos. 
Given the small variance across halos, each dimensionless radius is
associated with a physical value with a mean value and a standard deviation.
...
kpc.  
Finally, we also measure the alignment of the halo shape against the
stellar disc angular momentum. 
We measure the angular momentum vector using the $10\%$ oldest stellar
...
{\bf The main measurements are summarized in Table \ref{table:DMO} for the
DMO simulations and Table \ref{table:MHD} for the MHD simulations.
These results are computed at five different radii $R_{200}/16$,
$R_{200}/8$, $R_{200}/4$, $R_{200}/2$ and $R_{200}$.
In the following subsections we present and comment in greater detail
the results for the triaxiality, the halo-disc alignment and the
correlations with baryonic disc properties.}
...
the DMO sample the triaxiality has a median larger than $2/3$ (a
...
in the opposite direction with respect to DMO results.
There the median triaxiality is always smaller than $1/3$ (a typical
values that marks the transition from triaxiality to prolateness) and this
...
change between the triaxiality in the MHD and the DMO simulation.
The right panel in Figure \ref{fig:delta_triaxial_cumulative} shows
the cumulative distribution at the same radii as in the other panels.
There we observe that at $R_{200}$ all the halos have
reduced its triaxiality after including baryonic physics.
At smaller radii this general trend continues, although 2 haloes
(Au-11 and Au-17)  in our sample increased their triaxiality with MHD physics. 
...
The largest correlation shows with the halo triaxiality measured at
$0.12R_{200}$. 
...
{\bf Recently \cite{2017MNRAS.469..594B} reported that MW-like DM haloes can
be roughly splint into two distinct families: stalled and accreting.
...
constantly.
\cite{2017MNRAS.469..594B} also link the two populations to
differences in the dark matter filaments feeding the halos. 
Stalled halos at $z=0$ are embbeded in a single large thick filament, while
accreting halos are pierced by many thinner filaments. 
To what extent the twisting and high triaxiality present in the MHD
halos can be linked to this dicotomy?
...
The ages are estimated directly as the mean value in the stellar age
distribution for each component, while the formation redshift is
defined as the redshif at which the halo already has half of its $z=0$
mass. 
In the three cases we find a correlation between ages and twisting
that goes in the direction claimed by \cite{2017MNRAS.469..594B}:
components with early assembly (i.e. stalled) have a smaller degree of
twisting.
A detailed study with respect to the cosmic web location is beyond the
scope of our paper.}
...
The change of triaxiality with radius in our simulations cannot account
...
technique. 
The halos were simulated in two different setups:
...
function of radius and the degree of aligment between the angular
momentum in stellar disc and the dark matter halo. 
...
The rounding effect is more noticeable as one moves closer to the galactic
...
$\leq 0.5R_{200}$ and find that younger bulges and higher gas
